...
## Dataset Lab: curated benchmarks and safe tabular ingestion

The Dataset Lab supports a small, deterministic library of public research benchmarks (Breast Cancer Wisconsin Diagnostic, Pima Indians Diabetes, and Statlog Heart Disease) plus user-provided CSV, TSV, XLSX, XLS, and Parquet files. Built-in records include source, license, target semantics, version, and exact snapshot hashes; they are benchmark inputs for research demonstrations, not clinically validated datasets.

Uploads are inspected before registration. The inspector reports schema, numeric/categorical/boolean/datetime-like columns, missingness, infinite values, duplicates, constants, high-cardinality categories, identifier-like columns, and conservative target candidates. The user must explicitly select the target and positive class. Compatibility is reported as READY, WARNING, or BLOCKED with actionable checks. Q-Health remains a binary-classification research workflow: it does not silently merge multiclass labels, fabricate labels, fetch arbitrary URLs, or claim clinical or quantum advantage. See [`docs/dataset_ingestion.md`](docs/dataset_ingestion.md).

The inspector also returns an editable auto-detected metadata profile with origins for name, target, observed classes, positive/negative labels, version, domain, and source. Ambiguous target or class semantics are intentionally left for user confirmation; no AI or external inference service is used.
