...
export interface DatasetInspection {row_count:number;column_count:number;column_names:string[];numeric_columns:string[];categorical_columns:string[];boolean_columns:string[];datetime_like_columns:string[];unsupported_columns:string[];missing_values:Record<string,number>;infinite_values:Record<string,number>;duplicate_rows:number;duplicate_feature_rows:number;constant_features:string[];low_variance_features:string[];high_cardinality_categorical_features:string[];identifier_like_columns:string[];empty_columns:string[];possible_target_columns:string[];target_candidates:TargetCandidate[];target:string|null;target_classes:string[];class_distribution:Record<string,number>;schema_consistent:boolean;suspicious_target_proxy_columns:string[];transformations_applied:string[];compatibility:CompatibilityReport;file_format:string;filename:string;file_size_bytes:number}
