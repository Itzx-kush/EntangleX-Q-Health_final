...
export default function App() {
  ...
  return <IntroGate><ApplicationShell health={health} token={token} setToken={setToken} menu={menu} setMenu={setMenu} onApplyToken={() => {setSessionToken(token); setToken('');}}><PageTransition><Routes><Route path="/" element={<Dashboard/>}/><Route path="/datasets" element={<Datasets/>}/><Route path="/quality" element={<Quality/>}/><Route path="/preprocessing" element={<Preprocessing/>}/><Route path="/features" element={<FeatureSelection/>}/><Route path="/pca" element={<PCA/>}/><Route path="/training" element={<Training/>}/><Route path="/comparison" element={<Comparison/>}/><Route path="/quantum" element={<QuantumCircuit/>}/><Route path="/explainability" element={<Explainability/>}/><Route path="/prediction" element={<Prediction/>}/><Route path="/experiments" element={<Experiments/>}/><Route path="/experiments/:id" element={<ExperimentDetail/>}/><Route path="*" element={<div className="empty"><h1>Page not found</h1><NavLink to="/">Return to overview</NavLink></div>}/></Routes></PageTransition></ApplicationShell></IntroGate>;
}
