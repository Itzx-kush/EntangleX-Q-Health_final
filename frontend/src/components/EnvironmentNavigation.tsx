...
export function EnvironmentNavigation({open, onNavigate, collapsed, onToggleCollapsed, onHoverChange}: EnvironmentNavigationProps) {
  ...
  return <aside ref={navRef} id="environment-navigation" className={`sidebar environment-sidebar ${open ? 'open' : ''} ${collapsed ? 'sidebar-collapsed' : ''}`} aria-label="Research workspace navigation" aria-modal={open || undefined} onMouseEnter={() => onHoverChange?.(true)} onMouseLeave={() => onHoverChange?.(false)}>
