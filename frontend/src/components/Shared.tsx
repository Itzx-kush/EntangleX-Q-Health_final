...
import {AnimatedMetric,AnimatedSection,BlurText,GradientText,HoverLift,Reveal,ShinyText} from './reactbits';
...
export function MetricCard({label,value,detail,icon}:{label:string;value:ReactNode;detail?:string;icon?:ReactNode}){return <HoverLift><AnimatedMetric label={label} value={value} detail={detail} icon={icon}/></HoverLift>}
...
export function Loading(){return <Reveal><div className="glass-card rb-shimmer flex items-center gap-2 p-5 text-sm muted"><LoaderCircle size={15} className="animate-spin"/>Loading research data…</div></Reveal>}
