...
export function ResearchHero() {
  ...
  return <Motion variant="slide-up"><header className="research-hero split-hero">
...
export function RecentResearchActivity({summary, loading}: {summary?: ResearchSummary; loading: boolean}) {
  ...
  return <section className="research-activity" aria-labelledby="research-activity-title"><div className="research-section-heading"><div><p className="research-eyebrow plain">RESEARCH STUDIO</p><h2 id="research-activity-title">Recent activity</h2><p>Only experiments returned by the backend are shown.</p></div><Link className="text-link" to={environmentTarget('research-studio').path}>Open experiment log <ArrowRight size={14}/></Link></div>{loading ? <p className="loading-state motion-state state-loading" role="status" aria-label="Loading recent research activity"><span className="pulse-indicator" aria-hidden="true"/> Loading recent research activity...</p> : experiments.length ? <div className="research-activity-list">{experiments.map(experiment => <Link to={`/experiments/${experiment.id}`} key={experiment.id}><span className="activity-icon"><FlaskConical size={15} aria-hidden="true"/></span><span className="activity-copy"><strong>{shortId(experiment.id)}</strong><small>{dateTime(experiment.created_at)} · Dataset {shortId(experiment.dataset_id)}</small></span><StatusPill value={experiment.status}/><ArrowRight size={14} aria-hidden="true"/></Link>)}</div> : <EmptyState><FlaskConical size={21} aria-hidden="true"/><strong>No research activity available</strong><span>Completed or in-progress experiments will appear here when the backend returns them.</span><Link className="text-link" to={pageTarget('Datasets').path}>Open Data Lab <ArrowRight size={14}/></Link></EmptyState>}</section>;
}
