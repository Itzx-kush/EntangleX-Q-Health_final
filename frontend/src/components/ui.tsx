...
export function Button({className='',variant='primary',...props}:{variant?:'primary'|'outline'|'ghost'}&ButtonHTMLAttributes<HTMLButtonElement>){return <button className={cn('btn',variant==='primary'?'btn-primary':variant==='outline'?'btn-outline':'btn-ghost',className)} {...props}/>;}
...
export function Card({children,className='',title,description}:{children:ReactNode;className?:string;title?:string;description?:string}){return <section className={cn('glass-card p-5',className)}>{title&&<div className="mb-4"><h2 className="section-title">{title}</h2>{description&&<p className="section-copy mt-1">{description}</p>}</div>}{children}</section>;}
