...
type CommandResult = {kind: 'environment' | 'page' | 'context'; label: string; path: string; description: string; environmentName: string; Icon: LucideIcon};
...
export function CommandPalette({open, onClose}: {open: boolean; onClose: () => void}) {
  ...
  const {dataset, experiment, job} = useResearchWorkspace();
  ...
  const results = useMemo<CommandResult[]>(() => {
    ...
    const contextResults: CommandResult[] = [];
    if (dataset) contextResults.push({kind: 'context', label: `Open ${dataset.name}`, path: '/datasets', description: 'Current dataset context', environmentName: 'Current research', Icon: Search});
    if (experiment) contextResults.push({kind: 'context', label: `Open experiment ${shortId(experiment.id)}`, path: `/experiments/${experiment.id}`, description: `${experiment.config.models.map(model => modelLabels[model] || model).join(', ')} · ${experiment.status}`, environmentName: 'Current research', Icon: Search});
    if (job) contextResults.push({kind: 'context', label: 'Open current training job', path: '/training', description: `${job.status.replaceAll('_', ' ')} · backend-reported state`, environmentName: 'Current research', Icon: Search});
    return [...contextResults, ...envResults, ...pageResults];
  }, [dataset, experiment, job, normalized]);
