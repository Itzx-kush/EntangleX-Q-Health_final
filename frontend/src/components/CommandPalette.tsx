...
export function CommandPalette({open, onClose}: {open: boolean; onClose: () => void}) {
  ...
  return <Dialog open={open} onClose={onClose} title="Navigate Q-Health workspace"><div className="command-palette">
    <label className="command-search"><Search size={17} aria-hidden="true"/><span className="sr-only">Search workspace navigation</span><input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={onInputKeyDown} placeholder="Search environments and pages" autoComplete="off"/><kbd>Esc</kbd></label>
    {results.length ? <div className="command-results" role="listbox" aria-label="Workspace navigation results">{results.map((result, index) => {const Icon = result.Icon; const previous = results[index - 1]; const groupHeading = !previous || previous.kind !== result.kind; return <Fragment key={`${result.kind}-${result.path}`}>{groupHeading && <p className="command-group-label">{result.kind === 'environment' ? 'Environments' : 'Pages'}</p>}<button role="option" aria-selected={selected === index} className={`command-result ${selected === index ? 'selected' : ''}`} onMouseEnter={() => setSelected(index)} onClick={() => choose(result)}><span className="command-result-icon"><Icon size={16} aria-hidden="true"/></span><span><strong>{result.label}</strong><small>{result.kind === 'environment' ? 'Environment' : result.environmentName} <span>·</span> {result.description}</small></span><ArrowRight size={15} aria-hidden="true"/></button></Fragment>;})}</div> : <div className="command-empty"><Search size={20} aria-hidden="true"/><strong>No workspace locations found</strong><span>Try an environment or page name.</span></div>}
    <footer className="command-footer"><span><CornerDownLeft size={13} aria-hidden="true"/> Open</span><span><kbd>↑</kbd><kbd>↓</kbd> Move</span><span><kbd>⌘</kbd><kbd>K</kbd> Command</span></footer>
  </div></Dialog>;
}
