...
export function ResearchShell({children}:{children:ReactNode}){
  ...
  const [isOffcanvasViewport,setIsOffcanvasViewport]=useState(false);
  ...
  useEffect(()=>{const handler=(e:KeyboardEvent)=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();setPalette(true)}if(e.key==='Escape'){setPalette(false);setSettings(false);setMobile(false)}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler)},[]);
  ...
  useEffect(()=>{
    if(!isOffcanvasViewport)return;
    if(mobile){
      const closeButton=drawerRef.current?.querySelector<HTMLButtonElement>('.sidebar-close');
      closeButton?.focus();
      const previousOverflow=document.body.style.overflow;
      document.body.style.overflow='hidden';
      return()=>{document.body.style.overflow=previousOverflow};
    }
    if(document.activeElement?.closest('.research-sidebar'))menuButtonRef.current?.focus();
  },[isOffcanvasViewport,mobile]);
  ...
  return <div className={'research-shell '+(uiSettings.layout==='icon'?'is-collapsed ':'')+(uiSettings.layout==='offcanvas'?'is-offcanvas':'')}>
    <AmbientBackground/>
    <aside id="qhealth-navigation-drawer" ref={drawerRef} tabIndex={-1} aria-hidden={isOffcanvasViewport&&!mobile} className={'research-sidebar variant-'+uiSettings.sidebar+' '+(uiSettings.layout==='icon'?'is-collapsed ':'')+(mobile?'is-mobile-open ':'')+(uiSettings.layout==='offcanvas'?'is-offcanvas':'')} aria-label="Primary navigation">
