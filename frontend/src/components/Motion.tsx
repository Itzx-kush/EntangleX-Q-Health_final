...
export function PageTransition({children}: {children: ReactNode}) {
  ...
  const reduce = useReducedMotion();
  return <motion.div key={`${location.pathname}${location.search}`} className="route-transition" aria-live="polite" initial={{opacity: 0, y: reduce ? 0 : 6}} animate={{opacity: 1, y: 0}} transition={{duration: reduce ? 0 : .28, ease: [0.22, 1, 0.36, 1]}}>{children}</motion.div>;
}
